...
public class FireMapParallel {
    ...
    /** Advances one complete timestep by updating disjoint regions in parallel. */
    public StepResult parallelStep(int cutoff, ForkJoinPool pool) {
        if (cutoff < 1) {
            throw new IllegalArgumentException("cutoff must be at least one cell.");
        }
        prepareNextState();
        FireTask root = new FireTask(this, 1, 1, rows - 1, columns - 1, cutoff);
        StepResult result = pool.submit(root).join();
        completeStep();
        return result;
    }

    /** Updates a single half-open region using this map's configured mode. */
    StepResult updateRegion(int top, int left, int bottom, int right) {
        return updateRegion(mode, top, bottom, left, right);
    }
